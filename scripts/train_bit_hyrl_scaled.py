...
# -*- coding: utf-8 -*-
"""
BiT-HyRL 大规模重新训练脚本 (Scale-up Training)
=================================================

增大模型规模 (256/256/12/6) + 扩展数据集 + 完整攻击池的基线重新训练。
...
用法:
  python scripts/train_bit_hyrl_scaled.py --epochs 200
  python scripts/train_bit_hyrl_scaled.py --max-graphs 0 --epochs 100
  python scripts/train_bit_hyrl_scaled.py --resume src/train/v1/checkpoints/gnn_ppo_agent_scaled.pth
"""
import sys
import os
...
def parse_args():
    parser = argparse.ArgumentParser(
        description='BiT-HyRL Scaled-Up Training (Large Model + Large Dataset)'
    )
    ...
    parser.add_argument('--save-path', type=str, default=None,
                        help='模型保存路径 (默认: src/train/v1/checkpoints/gnn_ppo_agent_scaled.pth)')
    ...
    parser.add_argument('--aggregation', type=str, default='mean',
                        ...
                        help='多攻击奖励聚合方式 (默认: mean)')
    parser.add_argument('--sample-methods', type=int, default=5,
                        help='每次奖励计算随机采样的攻击方法数 (默认: 5, 0 表示使用全部)')
...
def main():
    ...
    if args.save_path is None:
        args.save_path = os.path.join(
            config.MODEL_DIR, 'gnn_ppo_agent_scaled.pth'
        )

    print("=" * 70)
    print("BiT-HyRL Scaled-Up Training")
    print("=" * 70)
    print(f"Mode: adversarial (scale-up)")
