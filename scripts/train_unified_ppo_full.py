...
def parse_args():
    ...
    parser.add_argument('--real-data-dir', type=str, default="",
                        help='真实网络数据目录，默认 "" 表示不使用真实网络训练，避免 testdata 泄露')
    parser.add_argument('--epochs', type=int, default=100)
    parser.add_argument('--lr', type=float, default=1e-4)
    parser.add_argument('--B-budget', type=int, default=3, help='拓扑微调预算（基础值，动态预算会在此基础上调整）')
    parser.add_argument('--dynamic-B', action='store_true', help='按图规模动态调整 B 预算')
    parser.add_argument('--B-scale', type=float, default=0.02, help='动态 B 的比例系数：B = base + int(N * B_scale)')
    parser.add_argument('--B-max', type=int, default=20, help='动态 B 的上限')
    ...
    parser.add_argument('--M-candidates', type=int, default=50, help='候选边池大小')
    parser.add_argument('--hidden-channels', type=int, default=256)
    parser.add_argument('--heads', type=int, default=8)
    parser.add_argument('--num-layers', type=int, default=5)
    parser.add_argument('--embed-dim', type=int, default=256)
    parser.add_argument('--dre-dim', type=int, default=0)
    parser.add_argument('--collect-per-epoch', type=int, default=150)
    ...
    parser.add_argument('--sample-methods', type=int, default=None,
    ...
    parser.add_argument('--no-adaptive-aggregation', action='store_true', help='禁用自适应聚合')
    parser.add_argument('--use-topology-reward', action='store_true', help='启用拓扑结构奖励（v5）')
    parser.add_argument('--no-topology-reward', action='store_true', help='禁用拓扑结构奖励')
    parser.add_argument('--w-topo', type=float, default=0.15, help='拓扑奖励总权重')
    parser.add_argument('--w-edge-btw-max', type=float, default=0.08, help='最大边 betweenness 惩罚权重')
    parser.add_argument('--w-edge-btw-std', type=float, default=0.04, help='边 betweenness 标准差惩罚权重')
    parser.add_argument('--w-degree-std', type=float, default=0.03, help='度标准差惩罚权重')
    parser.add_argument('--w-algebraic-conn', type=float, default=0.08, help='代数连通度奖励权重')
    parser.add_argument('--bimodal-weight', type=float, default=0.05, help='双峰偏离惩罚权重（v5 默认 0.05）')
    parser.add_argument('--focus-attacks', type=str, default='betweenness,pagerank', help='课程学习重点攻击方法')
    parser.add_argument('--seed', type=int, default=42)
    parser.add_argument('--save-path', type=str, default='src/train/v1/checkpoints/unified_ppo_agent_full_v4.pth')
...
def main():
    ...
    # 默认启用课程学习、自适应聚合、拓扑结构奖励
    if not args.use_curriculum and not args.no_curriculum:
        args.use_curriculum = True
    if not args.use_adaptive_aggregation and not args.no_adaptive_aggregation:
        args.use_adaptive_aggregation = True
    if not args.use_topology_reward and not args.no_topology_reward:
        args.use_topology_reward = True
    ...
    print("=" * 60)
    print("Unified PPO Full-Scale Training for BiT-HyRL")
    print("=" * 60)
    dynamic_b_str = f" (dynamic B={args.dynamic_B}, scale={args.B_scale}, max={args.B_max})" if args.dynamic_B else ""
    print(f"Epochs: {args.epochs}, B-budget: {args.B_budget}{dynamic_b_str}, K-ratio: {args.K_ratio}")
    print(f"M-candidates: {args.M_candidates}, LR: {args.lr}")
    print(f"Topology reward: {args.use_topology_reward}, w_topo={args.w_topo}, focus={args.focus_attacks}")
    ...
    focus_attacks = [m.strip() for m in args.focus_attacks.split(',') if m.strip()]
    ...
    def reward_fn(G_final, controllers, G_original, epoch=None, total_epochs=None):
        return calculate_unified_reward(
            ...
            use_topology_reward=args.use_topology_reward,
            w_topo=args.w_topo,
            w_edge_betweenness_max=args.w_edge_btw_max,
            w_edge_betweenness_std=args.w_edge_btw_std,
            w_degree_std=args.w_degree_std,
            w_algebraic_connectivity=args.w_algebraic_conn,
            bimodal_weight=args.bimodal_weight,
    ...
    for epoch in range(start_epoch, total_epochs):
        ...
        for i, G in enumerate(sampled):
            ...
            if args.dynamic_B:
                B = min(args.B_max, args.B_budget + int(G.number_of_nodes() * args.B_scale))
            else:
                B = args.B_budget
